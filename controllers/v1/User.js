...
const fprint = function (userUpdates, res){
    ...
    // to declare some path to store your converted image
        ...
        const imgdata = userUpdates.fingerprint;
    ...
    if(validAdmin(user)){
       let q={super: false, deleted: false};
        ...
        if(count){
            ...
            let agresp =  await db.collection('users').aggregate([
                {"$match":q},
                {
                $lookup: {
                        from: "fingerprints",
            ...
            if (agresp.length) res.json({success:agresp})
